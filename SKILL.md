...
- `renderRankingFondos()` *(v1.3)* — card en pestaña Fondos (render ~línea 1267): ranking ordenado DESC con medallas 🥇🥈🥉. Toggle métrica `tasa`/`interes` + período `mes`/`anio` + selectores año/mes dinámicos (solo se ofrecen los que tienen datos en HISTORIAL). Para ganancias convierte cada fila con `toUSD()`. Para tasa anual usa rendimiento compuesto `∏(1+t/100)-1`. Si ningún fondo tiene datos en el período, muestra empty state.
- `setRankMetric(m)` / `setRankPeriodo(p)` *(v1.3)* — cambian state global y re-renderizan solo el ranking
- **State global del ranking:** `rankMetric` / `rankPeriodo` / `rankMes` / `rankAnio`. Para tocar el ranking: esas 4 variables + `renderRankingFondos()` + los 2 handlers. Nada más.
...
⚠️ **Sync SKILL.md — 2 ubicaciones, deben quedar IDÉNTICAS** si hay cambios estructurales:
1. `C:\Users\segur\.claude\skills\especialista-inversiones-J-A-S\SKILL.md` (nivel usuario)
2. `C:\Users\segur\Documents\GitHub\inversiones-J-A-S\SKILL.md` (repo, se commitea)

Este SKILL.md es la **memoria única** del proyecto: no hay nodos en el Claude Memory Engine (el CME es exclusivo de SASINS). Todo lo durable del proyecto se escribe acá.
...
⚠️ **Lección aprendida v1.4 (11 jun) — revisar el archivo antes del push**: en esa jornada se descartó, sin commitear, un bloque duplicado donde `totalGanUSD` quedaba **declarado 2 veces dentro de `renderRankingFondos`** — un `SyntaxError` que habría matado la app ENTERA si se pusheaba (single-file: un error de sintaxis y no carga nada). En una app de un solo archivo, un parche mal pegado es un apagón total, no un bug parcial. También se borraron los `_diff*.txt` que dejaron los agentes de revisión: **no commitear archivos temporales de auditoría**.
